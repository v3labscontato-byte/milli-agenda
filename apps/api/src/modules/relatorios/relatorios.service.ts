...
export class RelatoriosService {
  ...
  private async ensureGoalsTableInline() {
    ...
    try {
      await this.db.$executeRawUnsafe(
        ...
        'id text NOT NULL, "tenantId" text NOT NULL, tipo text NOT NULL, periodo text NOT NULL,' +
        'valor numeric(12,2) NOT NULL, "dataInicio" timestamp NOT NULL, "dataFim" timestamp NOT NULL,' +
  ...
  async createGoal(tenantId: string, dto: { tipo: string; periodo: string; valor: number; dataInicio: string; dataFim: string }) {
    await this.ensureGoalsTableInline()
    const id = randomUUID()
    const now = new Date()
    ...
    try {
      await this.db.$executeRawUnsafe(
        ...
        id, tenantId, dto.tipo, dto.periodo, String(dto.valor), dataInicio, dataFim, now, now,
      )
