    ...
    <div className="flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 pb-2">
        <button
          ...
          className="flex h-9 w-9 items-center justify-center rounded-full text-[#475569] hover:bg-[#F1F5F9] active:bg-[#E2E8F0]"
        >
          <ChevronLeft size={18} aria-hidden="true" />
        </button>
      ...
      <div className="grid grid-cols-7 px-3">
        {WEEKDAYS.map((d) => (
          <div key={d} className="py-1 text-center text-[11px] font-semibold text-[#94A3B8]">{d}</div>
        ))}
      </div>
      ...
      {/* Time slots */}
      {selectedDate && (
        <div className="mt-5 border-t border-[#F1F5F9] px-4 pt-4">
          <p className="mb-3 text-[13px] font-semibold text-[#475569]">
            Horários — {formatDateHeader(selectedDate)}
          </p>
          {slots.length === 0 ? (
            <p className="py-4 text-center text-[13px] text-[#94A3B8]">Sem horários disponíveis neste dia.</p>
          ) : (
