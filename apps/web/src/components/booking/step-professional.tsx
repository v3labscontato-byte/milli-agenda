  ...
  return (
    <div className="flex flex-col">
      ...
      <button
        type="button"
        onClick={onBack}
        className="flex items-center gap-1 px-4 py-3 text-[14px] text-[#2563EB]"
      >
        <ChevronLeft size={18} aria-hidden="true" />
        <span className="font-medium">{service.emoji} {service.name}</span>
        <span className="ml-1 text-[#94A3B8]">· {formatDuration(service.durationMins)} · {formatPrice(service.price)}</span>
      </button>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-[13px] text-[#64748B]">
                {pro.role}
                {pro.reviews > 0 && (
                  <span className="text-[#94A3B8]"> · {pro.reviews} avaliações</span>
                )}
              </p>
